...
# Style LoRAs (Myst / MysticXXX / etc.) are NOT prefetched here.
# Jobs pull them at runtime via handler lora_url / loras[] + optional hf_token / HF_TOKEN.
# Base MiniMax weights must already be baked into the image (do not slim-bake).

# Realism People: optional volume link below; else job-time Hub download when realism_lora=true.
